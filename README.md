...
Local-first account recovery and exposure-risk assistant for macOS and Windows.
...
flowchart TD
  ...
  OAuth --> Classifier
  Classifier --> Findings["Account risk signals"]
  Findings --> Reset["Reset workflow builder\nverified-link Playwright opener"]
...
The `breach-check` command uses the paid HIBP breached-account endpoint for an email-address lookup. HIBP breached-account checks require an API key and send the searched email address to HIBP. This is the optional paid path. The tool stores the HIBP key in the OS credential store and logs only breach counts, not the API key.
...
This tells you whether HIBP returns breach records for the email address. It does not prove that every linked service account is compromised, and it cannot detect private breaches HIBP does not have.
...
The `exposure-plan` command is a scoped review alternative to trying to search the whole web for a password. It combines:
...
It does not crawl paste sites, dark-web sources, criminal forums, or random web pages for plaintext passwords. That boundary is deliberate: those sources can expose you further, produce unreliable data, and create legal/security risk. The app uses HIBP results and authorized mailbox evidence to prioritize accounts for review.
...
Bitwarden and NordPass can store and sync passkeys for websites that support them, but this CLI cannot create or enroll passkeys for third-party websites on your behalf. When account review leads you to rotate a password, use the service's official passkey enrollment flow manually when available, then save that passkey in your chosen password manager.

Use `passkey-guidance --service <name>` for official enrollment guidance. The GUI also includes a passkey guidance panel.
...
- `rotate` masks the five generated choices by default and does not print the selected password unless you explicitly opt in. `--copy-selected` is the recommended path: it copies the selected password to the clipboard and clears it after 60 seconds when the platform supports it, but only if the clipboard still contains that copied password. `--reveal-selected` prints the selected password in the terminal only when you accept that screen and scrollback exposure risk.
...
- Helps reduce risk from reused passwords, stale exposed credentials, forgotten breach alerts, and vault drift between Bitwarden and NordPass.
...
```bash
cd /Users/Antman/Desktop/AccountRecoveryGuard
python3 -m venv .venv
...
```powershell
cd $HOME\Desktop\AccountRecoveryGuard
py -3.11 -m venv .venv
...
- Follow a review checklist for email connection, local scan, account review, password exposure check, and vault sync.
- See the review boundary on first launch: authorized mailbox evidence plus free HIBP k-anonymous checks, not unsafe whole-web or dark-web crawling.
- Label who the scan is for, using quick choices like `Me` or `Second person`, without creating a stored profile.
- Review one mailbox at a time; run a separate scan for a second person only when they are present and have asked you to help.
...
- Review a clear next review action plus the accounts needing attention found by the scan.
...
- Read a plain-English review plan that separates what the app knows, what it cannot prove, the next review action, and the guardrail that keeps recovery local-first.
- Interpret scan results with uncertainty: mailbox findings are risk signals, and "no urgent alerts" does not prove every password is risk-free.
- Extract readable `http`/`https` links from HTML email buttons so security alerts with button-only reset links are not missed.
...
- Review the account evidence first, then rotate with masked choices only when the signal supports it; prepare vault sync only after confirming the password was changed on the real service and the new password works.
...
After a scan exists, the dashboard shows an account risk summary, accounts needing attention, vault sync status, cleanup reminders, and secondary advanced tools for troubleshooting.
...
```bash
arg secret old-reused-password
arg pwned-password --password-secret old-reused-password --confirm-old-or-reused
arg secret-delete old-reused-password
```

This is the free check. It uses the HIBP Pwned Passwords range API and does not require a HIBP API key. Do not check a new generated password; save new passwords directly in your vaults. Delete that temporary old-password secret after the check.

Create an exposure review plan from mailbox scan JSON plus the free password check:

```bash
arg exposure-plan \
  --email you@example.com \
  --password-secret old-reused-password \
...
The exposure plan prioritizes accounts to review. If the checked password appears in HIBP Pwned Passwords, use your password manager or memory to identify where you reused it, then rotate only those accounts.

After reviewing account evidence, change one password with five local password choices:
...
The command shows five masked generated passwords, copies only the one you choose when `--copy-selected` is present, opens the reset link if requested in a no-download recovery browser, waits for you to complete MFA/CAPTCHA/passkey prompts manually, then only writes vaults after you confirm the new password works and type `ROTATED`. Use `--reveal-selected` only when clipboard copy is unavailable and you accept terminal exposure risk.
...
The GitHub Actions workflow at `.github/workflows/build-account-recovery-guard.yml` builds both artifacts from the repository root on native runners and uploads them as workflow artifacts. The macOS DMG includes an `Applications` shortcut plus this README; the Windows artifact includes `README-Windows.txt` with install-warning context. Use the workflow when you need to recreate the Windows `.exe` from macOS.

Latest local macOS package check:

- Recreated: 2026-06-15 UTC
- Artifact: `dist/AccountRecoveryGuard-macOS.dmg`
- SHA-256: `237e621b8681dc2bf6bce749e86d4c58f2986222d3476883ab66bffdaf06a3a7`
- Signing status: `unsigned-development`

Windows `.exe` packaging must run on Windows or a Windows GitHub Actions runner because PyInstaller does not cross-build Windows executables from macOS.
...
AccountRecoveryGuard/
  ...
  src/account_recovery_guard/
    ...
    exposure.py                     Exposure review report combining mailbox evidence and HIBP results.
    ...
    reset_orchestrator.py           Verified-link password reset workflow and Playwright opener.
  ...
  tests/
    ...
    test_exposure.py                Verifies exposure-review recommendations and boundaries.
    ...
    test_live_vault.py              Verifies live-vault preflight and marked test entry construction.
...
- NordPass personal vault writes are not fully automatable through an official public CRUD API. CSV import/export is the supported path used here today.

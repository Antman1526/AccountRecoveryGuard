...
def _credential_store_check() -> ReadinessCheck:
    try:
        ...
        return ReadinessCheck(
            "OS credential store",
            "action_needed",
            "The Python keyring backend is unavailable; secrets cannot be stored through the OS credential store until keyring works.",
        )
    return ReadinessCheck(
        "OS credential store",
        "ready",
        "Secrets can be stored and deleted through macOS Keychain or Windows Credential Manager via keyring; delete temporary password-check secrets after use.",
    )
...
def _hibp_paid_check(hibp_secret_name: str | None) -> ReadinessCheck:
    ...
    return ReadinessCheck(
        "HIBP email-breach lookup",
        "paid_optional",
        "HIBP API key secret was not found. Add it only if you choose to use paid HIBP email-breach lookup.",
    )

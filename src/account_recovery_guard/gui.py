...
from .gui_workflow import (
    ...
    password_exposure_display_message,
    password_exposure_prompt_lines,
    password_exposure_result_summary,
...
def main() -> int:
    ...
    class MainWindow(QMainWindow):
        def __init__(self) -> None:
            ...
            self._password_exposure_triage_widgets = []
        ...
        def _connect_email_page(self) -> QScrollArea:
            ...
            layout.addWidget(
                StepHeader(
                    "Connect email for review",
                    "Scan account and security emails to find websites tied to you and accounts that may need attention.",
                    "Step 1 of 3",
                )
            )
            layout.addWidget(self._exposure_boundary_card())
            layout.addWidget(self._helper_mode_card())
            ...
            safe_scope = Card("Recovery review scope")
            for line in review_scope_lines():
            ...
            checklist = Card("Review checklist")
        ...
        def _exposure_boundary_card(self) -> Card:
            card = Card("What this app can check")
            for item in exposure_boundary_rows():
                row = QFrame()
        ...
        def _helper_mode_card(self) -> Card:
            card = Card("Who are you helping review?")
            self.review_subject_choice_label = self._body_label(
                f"Current choice: {self.state.review_subject_label}.",
                "listText",
            )
            card.body.addWidget(self.review_subject_choice_label)
            card.body.addWidget(
                self._body_label(
                    "This keeps the flow simple for one person or a second person sitting with you. "
                    "Each mailbox gets its own scan and review. "
                    "Only scan a mailbox you own or are explicitly helping with while that person is present.",
                    "listText",
                )
            )
            button_row = QHBoxLayout()
            for label, button_text in (("Me", "Review my accounts"), ("Second person", "Help someone present")):
                button = QPushButton(button_text)
                button.setObjectName("secondaryButton")
                button.setCursor(Qt.CursorShape.PointingHandCursor)
                button.clicked.connect(lambda checked=False, selected=label: self._set_review_subject_choice(selected))
                button_row.addWidget(button)
            button_row.addStretch(1)
            card.body.addLayout(button_row)
            return card

        def _set_review_subject_choice(self, label: str) -> None:
            self.state = self.state.with_review_subject(label)
            if hasattr(self, "review_subject_choice_label"):
                self.review_subject_choice_label.setText(f"Current choice: {self.state.review_subject_label}.")
            if hasattr(self, "setup_person_label"):
                self.setup_person_label.setText(self.state.review_subject_label)
        ...
        def _scan_consent_page(self) -> QScrollArea:
            ...
            self.setup_person_label = QLineEdit(self.state.review_subject_label)
            ...
            setup.body.addWidget(
                self._body_label(
                    "Review one mailbox at a time. For a second person, start a separate scan only when they are "
                    "present and have asked you to help. "
                    "Do not use this app to investigate someone else's accounts without permission.",
                    "listText",
                )
            )
            ...
            self.setup_scan_consent = QCheckBox(
                "I have permission to scan this authorized mailbox."
            )
            self.setup_scan_consent.setObjectName("consentCheck")
            self.setup_scan_consent.setCursor(Qt.CursorShape.PointingHandCursor)
            self.setup_scan_consent.stateChanged.connect(lambda: self._update_start_scan_button_state())
            ...
            self.setup_second_person_consent.stateChanged.connect(lambda: self._update_start_scan_button_state())
            ...
            self.setup_start_scan_button = start
            ...
            self._update_provider_setup_visibility()
            self._update_second_person_consent_visibility()
            self._update_start_scan_button_state()
            return page
        ...
        def _update_start_scan_button_state(self) -> None:
            if not hasattr(self, "setup_start_scan_button"):
                return
            permission_confirmed = bool(
                hasattr(self, "setup_scan_consent") and self.setup_scan_consent.isChecked()
            )
            second_person_required = bool(
                hasattr(self, "setup_person_label") and requires_second_person_consent(self.setup_person_label.text())
            )
            second_person_confirmed = bool(
                hasattr(self, "setup_second_person_consent") and self.setup_second_person_consent.isChecked()
            )
            self.setup_start_scan_button.setEnabled(
                scan_start_ready(permission_confirmed, second_person_required, second_person_confirmed)
            )
        ...
        def _render_results(self) -> None:
            ...
            self.results_layout.addWidget(
                StepHeader(
                    self.state.review_subject_prefix + summary.headline,
                    summary.attention_text,
                    "Step 3 of 3",
                )
            )
            self.results_layout.addWidget(self._review_plan_card())
        ...
        def _render_empty_results(self, summary: ScanSummary | None = None) -> None:
            if hasattr(self, "results_layout"):
                ...
                self.results_layout.addWidget(self._review_plan_card(summary))
                linked_accounts = self._linked_accounts_card(summary)
                if linked_accounts is not None:
                    self.results_layout.addWidget(linked_accounts)
        ...
        def _linked_accounts_card(self, summary: ScanSummary | None) -> Card | None:
            if summary is None or not summary.discovered_accounts:
                return None
            card = Card("Linked accounts from mailbox evidence")
            card.body.addWidget(StatusPill("not exposure findings", "safe"))
            card.body.addWidget(self._body_label(summary.linked_accounts_explanation, "listText"))
            for index, account in enumerate(summary.discovered_accounts[:8], start=1):
                ...
                detail = QLabel(
                    f"{account.sender_domain} - {account.confidence} confidence - {reasons}. "
                    "Inventory only; rotate only if another risk signal applies."
                )
                detail.setObjectName("listText")
                detail.setWordWrap(True)
                text_col.addWidget(title)
        ...
        def _render_rotation_panel(self) -> None:
            ...
            self.rotation_layout.addWidget(
                StepHeader(
                    f"Review {account.service_name} password change",
                    "Use a masked generated password only if the reviewed evidence supports rotation.",
                    "Step 4 of 5",
                )
            )
            ...
            self.rotation_status_label = self._body_label(
                "After deciding a password change is needed, select a masked choice, use the official site or verified reset page, then copy the selected password.",
                "listText",
            )
            ...
            copy_guard.stateChanged.connect(
                lambda state, button=copy, selected=session.selected_index is not None: button.setEnabled(
                    rotation_copy_ready(state != 0, selected)
                )
            )
            ...
            if account.reset_link and account.reset_link_is_trusted:
                reset = QPushButton("Open verified reset browser")
                reset.setObjectName("secondaryButton")
                reset.setCursor(Qt.CursorShape.PointingHandCursor)
                reset.clicked.connect(lambda checked=False, selected=account: self._open_verified_reset_browser(selected))
            ...
            confirmed.stateChanged.connect(
                ...
                checked=verified,
                button=sync_vaults,
                selected=session.selected_index is not None: button.setEnabled(
                    vault_sync_ready(changed.isChecked(), checked.isChecked(), selected)
                )
            )
        ...
        def _open_verified_reset_browser(self, account: AccountReview) -> None:
            ...
            if self._reset_browser_thread is not None:
                if hasattr(self, "rotation_status_label"):
                    self.rotation_status_label.setText("A verified reset browser is already open.")
                return
            if hasattr(self, "rotation_status_label"):
                self.rotation_status_label.setText(
                    "Opening verified reset browser with downloads blocked. Close it when the reset is complete."
                )
            thread = QThread(self)
        ...
        def _dashboard_page(self) -> QScrollArea:
            ...
            self.dashboard_checklist_card = Card("Review checklist")
        ...
        def _refresh_dashboard(self) -> None:
            ...
            if summary is None:
                ...
                if hasattr(self, "dashboard_plan_label"):
                    plan = build_review_plan(None, self.state.password_exposure_count, self.state.vault_status)
            ...
            self.dashboard_summary_label.setText(
                f"{self.state.review_subject_prefix}{summary.headline}. {summary.attention_text} {vault_text}{cleanup_text}"
            )
            if hasattr(self, "dashboard_plan_label"):
                plan = build_review_plan(summary, self.state.password_exposure_count, self.state.vault_status)
                self.dashboard_plan_label.setText(f"{plan.headline}: {plan.next_action}")
            if hasattr(self, "dashboard_vault_status_label"):
        ...
        def _password_exposure_card(self, title: str) -> Card:
            ...
            card.body.addWidget(
                self._body_label(
                    password_exposure_result_summary(self.state.password_exposure_count),
                    "warningText"
                    if self.state.password_exposure_count and self.state.password_exposure_count > 0
                    else "listText",
                )
            )
            ...
            triage_steps = reused_password_triage_steps(self.state.password_exposure_count)
            triage_title = QLabel("Reused password triage")
            triage_title.setObjectName("sectionTitle")
            triage_title.setVisible(bool(triage_steps))
            card.body.addWidget(triage_title)
            triage_note = self._body_label(
                "\n".join(f"{index}. {step}" for index, step in enumerate(triage_steps, start=1)),
                "listText",
            )
            triage_note.setVisible(bool(triage_steps))
            card.body.addWidget(triage_note)
            self._password_exposure_triage_widgets.append((triage_title, triage_note))
        ...
        def _review_plan_card(self, summary: ScanSummary | None = None) -> Card:
            ...
            for label, text in (
                ...
                ("Next review action", plan.next_action),
        ...
        def _home_page(self) -> QScrollArea:
            page = self._scroll_page(
                "Review risky account signals with both vaults kept in sync",
        ...
        def _mail_page(self) -> QScrollArea:
            page = self._scroll_page(
                "Scan email for risky account signals",
                "Build a scoped scan command for Gmail, Outlook, or IMAP. Secrets are referenced by name and read from the OS credential store.",
            )
            layout = page.content_layout  # type: ignore[attr-defined]
            panel = QFrame()
        ...
        def _advanced_rotation_page_unreachable_from_first_run(self) -> QScrollArea:
            page = self._scroll_page(
                "Review one risky account before rotating",
                "Generate choices only after the account evidence supports a password change, then copy only the chosen password.",
            )
            layout = page.content_layout  # type: ignore[attr-defined]
            panel = QFrame()
            panel.setObjectName("panel")
            ...
            reveal_selected = QCheckBox("Unsafe: reveal selected password in terminal")
            official_page_confirmed = QCheckBox("I am on the official website, official app, or verified reset page")
            official_page_confirmed.setObjectName("consentCheck")
            official_page_confirmed.setCursor(Qt.CursorShape.PointingHandCursor)
            choices = QListWidget()
            choices.setObjectName("choiceList")
            ...
            def update_preview() -> None:
                command = build_command_preview(
                    "rotate",
                    {
                        ...
                        "reveal_selected": reveal_selected.isChecked(),
                    },
                )
                preview.setText(command)
            ...
            def generate() -> None:
                ...
                result.setText(
                    "Five choices generated. After deciding a password change is needed, select one, "
                    "use the official site or verified reset page, then copy the selected password."
                )

            def copy_password() -> None:
                if not official_page_confirmed.isChecked():
                    result.setText("Confirm you are on the official website, official app, or verified reset page before copying.")
                    return
            ...
            reveal_selected.stateChanged.connect(update_preview)
            ...
            form.addWidget(reveal_selected, 6, 1)
            form.addWidget(official_page_confirmed, 7, 1)
            ...
            official_page_confirmed.stateChanged.connect(lambda state, button=copy_button: button.setEnabled(state != 0))
            copy_button.clicked.connect(copy_password)
            button_row = QHBoxLayout()
            button_row.addWidget(generate_button)
            button_row.addWidget(copy_button)
            button_row.addStretch(1)
            form.addLayout(button_row, 8, 1)
            form.addWidget(QLabel("Password choices"), 9, 0)
            form.addWidget(choices, 9, 1)
            form.addWidget(QLabel("Status"), 10, 0)
            form.addWidget(result, 10, 1)
            form.addWidget(QLabel("CLI equivalent"), 11, 0)
            form.addWidget(preview, 11, 1)
            form.addWidget(self._copy_button("Copy rotate command", preview.toPlainText), 12, 1, alignment=Qt.AlignmentFlag.AlignRight)
            layout.addWidget(panel)
            update_preview()
            return page
        ...
        def _security_page(self) -> QScrollArea:
            ...
            password_check_box = QGroupBox("Check one old or reused password")
            ...
            password_triage_steps = reused_password_triage_steps(self.state.password_exposure_count)
            password_triage_note = self._body_label(
                "\n".join(f"{index}. {step}" for index, step in enumerate(password_triage_steps, start=1)),
                "warningText",
            )
            self.password_exposure_triage_note = password_triage_note
            password_triage_note.setVisible(bool(password_triage_steps))
            ...
            password_check_layout.addWidget(password_triage_note, 3, 0, 1, 2)
            password_check_layout.addWidget(password_check, 4, 1, alignment=Qt.AlignmentFlag.AlignRight)
            layout.addWidget(password_check_box)

            exposure_box = QGroupBox("Exposure review plan")
                ...
                exposure_preview.setText(
                    build_command_preview(
                        "exposure-plan",
                        {
                            ...
                            "confirm_old_or_reused": bool(exposure_password_secret.text().strip()),
            ...
            paid_note = QLabel(
                ...
                "only if you decide the paid email-address lookup is worth it."
            )
            ...
            cleanup_note = QLabel(
                "Delete the temporary password-check secret after running the command. "
                "Use: arg secret-delete <secret-name>"
            )
            cleanup_note.setObjectName("warningText")
            cleanup_note.setWordWrap(True)
            exposure_layout.addWidget(cleanup_note, len(exposure_fields) + 2, 0, 1, 2)
            exposure_layout.addWidget(QLabel("Command"), len(exposure_fields) + 3, 0)
            exposure_layout.addWidget(exposure_preview, len(exposure_fields) + 3, 1)
            exposure_layout.addWidget(
                self._copy_button("Copy exposure command", exposure_preview.toPlainText),
                len(exposure_fields) + 4,
                1,
                alignment=Qt.AlignmentFlag.AlignRight,
            )
        ...
        def _finish_password_exposure_check(self, result) -> None:
            ...
            if status_label is not None:
                status_label.setText(password_exposure_display_message(result.count, result.user_message))
            if action_button is not None:
                action_button.setEnabled(False)
            self._refresh_password_exposure_triage_guidance()
            self._refresh_dashboard()

        def _refresh_password_exposure_triage_guidance(self) -> None:
            triage_steps = reused_password_triage_steps(self.state.password_exposure_count)
            triage_text = "\n".join(f"{index}. {step}" for index, step in enumerate(triage_steps, start=1))
            visible = bool(triage_steps)
            live_widgets = []
            for triage_title, triage_note in self._password_exposure_triage_widgets:
                try:
                    triage_title.setVisible(visible)
                    triage_note.setText(triage_text)
                    triage_note.setVisible(visible)
                except RuntimeError:
                    continue
                live_widgets.append((triage_title, triage_note))
            self._password_exposure_triage_widgets = live_widgets
            if hasattr(self, "password_exposure_triage_note"):
                try:
                    self.password_exposure_triage_note.setText(triage_text)
                    self.password_exposure_triage_note.setVisible(visible)
                except RuntimeError:
                    pass

...
SETUP_DETAIL_PROVIDER_PREP_FAILED = "provider_prep_failed"
...
def build_provider_or_error(settings: MailProviderSettings) -> tuple[MailProvider | None, UserFacingSetupError | None]:
    if settings.provider == MailProviderChoice.GMAIL:
        ...
        if settings.gmail_advanced_oauth:
            ...
            try:
                ...
                return _provider_prep_failed_error("Gmail OAuth")
        ...
        try:
            ...
            return _provider_prep_failed_error("Gmail")
    ...
    if settings.provider == MailProviderChoice.OUTLOOK:
        ...
        try:
            ...
            return _provider_prep_failed_error("Outlook")
    ...
    try:
        ...
        return _provider_prep_failed_error("IMAP")


def _provider_prep_failed_error(provider_name: str) -> tuple[None, UserFacingSetupError]:
    return None, UserFacingSetupError(
        user_message=f"{provider_name} setup could not be prepared. Check setup and try again.",
        technical_details=SETUP_DETAIL_PROVIDER_PREP_FAILED,
    )
    ...
    def check_password(self, password: str, confirmed_old_or_reused: bool = False) -> GuiPasswordExposureResult:
        ...
        if previous_result is not None:
            if previous_result.count and previous_result.count > 0:
                previous_summary = (
                    f"Previous result: found {previous_result.count} time(s). Review where you reused it, "
                    "then rotate only those accounts."
                )
        ...
        if count > 0:
            result = GuiPasswordExposureResult(
                ...
                user_message=(
                    ...
                    "Do not reuse it; review where you reused it, then rotate only those accounts."
                ),

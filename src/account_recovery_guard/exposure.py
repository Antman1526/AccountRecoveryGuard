...
from .models import AccountRiskFinding, DiscoveredAccount, Severity


SAFE_EXPOSURE_BOUNDARY = (
    "Exposure review checks use authorized mailbox evidence, optional HIBP email lookup, and HIBP k-anonymous "
    "password checks. The app does not crawl paste sites, dark-web sources, criminal forums, or random web "
    "pages for plaintext passwords."
)
...
def build_exposure_report(
    ...
    if password_pwned_count and password_pwned_count > 0:
        for row in service_rows.values():
            ...
            assert isinstance(reasons, set)
            reasons.add(
                "checked password appears in HIBP Pwned Passwords; confirm this account used that password before rotating"
            )

...
def main() -> None:
    parser = argparse.ArgumentParser(prog="arg", description="Local account recovery and exposure-risk assistant.")
    ...
    secret_delete = sub.add_parser("secret-delete", help="Delete an OS-keychain secret by name")
    ...
    breach = sub.add_parser("breach-check", help="Check an email address with the paid HIBP email-breach lookup")
    ...
    pwned_password = sub.add_parser(
        "pwned-password",
        help="Check an old or reused password against the free HIBP Pwned Passwords k-anonymity API",
    )
    pwned_password.add_argument("--password-secret", required=True)
    pwned_password.add_argument(
        "--confirm-old-or-reused",
        action="store_true",
        help="Required confirmation: only check an old or reused password, never a new generated password",
    )
    ...
    exposure = sub.add_parser(
        "exposure-plan",
        help="Build a scoped review plan from mailbox findings and HIBP checks",
    )
    ...
    exposure.add_argument("--password-secret", help="Optional OS-keychain secret containing an old or reused password to check with HIBP k-anonymity")
    exposure.add_argument(
        "--confirm-old-or-reused",
        action="store_true",
        help="Required with --password-secret: only check an old or reused password, never a new generated password",
    )
    ...
    workflow.add_argument(
        "--open",
        action="store_true",
        help="Open verified extracted reset link in Playwright; otherwise use the official site or app",
    )

    rotate = sub.add_parser("rotate", help="After review, pick from five passwords, use the official reset flow, then update vaults")
    ...
    rotate.add_argument(
        "--open",
        action="store_true",
        help="Open verified reset link in Playwright before vault write; otherwise use the official site or app",
    )
    rotate.add_argument("--reveal-all", action="store_true", help="Unsafe: print every generated plaintext choice")
    rotate.add_argument(
        "--reveal-selected",
        action="store_true",
        help="Unsafe: print only the selected plaintext password after selection",
    )
    ...
    live = sub.add_parser("vault-live-test", help="Test Bitwarden write and NordPass import staging with a marked test entry")
    ...
    passkey = sub.add_parser("passkey-guidance", help="Show official passkey enrollment guidance")
    ...
    if args.command == "secret":
        ...
        print("Stored secret in the OS credential store.")
    elif args.command == "secret-delete":
        _delete_secret(args.name)
        print("Deleted secret from the OS credential store if it existed.")
...
def _missing_secret_message(label: str, *, include_store_hint: bool = False) -> str:
    message = f"{label} secret was not found."
    if include_store_hint:
        message += " Store it with: arg secret <secret-name>"
    return message
...
def _scan_imap(args: argparse.Namespace) -> None:
    ...
    if not password:
        raise SystemExit(_missing_secret_message("IMAP", include_store_hint=True))
...
def _print_findings(findings, as_json: bool) -> None:
    ...
    if not findings:
        print(
            "No mailbox risk signals were found in this scan. This does not prove every account is risk-free; "
            "keep passwords unique and check old reused passwords with HIBP k-anonymity."
        )
        return
    print(
        "Mailbox risk signals for review. These alerts are not proof of account takeover. "
        "Use the official website or app before changing passwords."
    )
...
def _redact_url_for_display(url: str) -> str:
    ...
    netloc = _safe_display_netloc(parts)
    if not netloc:
        return "redacted reset link"
    display = urlunsplit((parts.scheme, netloc, parts.path, "", ""))
    if parts.query:
        display += "?<redacted>"
    if parts.fragment:
        display += "#<redacted>"
...
def _safe_display_netloc(parts) -> str:
...
def _discover_imap(args: argparse.Namespace) -> None:
    password = get_secret(args.secret_name)
    if not password:
        raise SystemExit(_missing_secret_message("IMAP"))
...
def _scan_gmail_app_password(args: argparse.Namespace) -> None:
    password = get_secret(args.secret_name)
    if not password:
        raise SystemExit(_missing_secret_message("Gmail app-password", include_store_hint=True))
    scanner = ImapEmailScanner(
        ImapMailboxConfig(
            host="imap.gmail.com",
            username=args.username,
...
def _breach_check(args: argparse.Namespace) -> None:
    ...
    if not api_key:
        raise SystemExit(_missing_secret_message("HIBP API key"))
    breaches = _breaches_for_account_or_exit(HibpBreachChecker(api_key), args.email)
    AuditLogger().write("hibp_breach_check", email=args.email, breach_count=len(breaches))
    if args.json:
        print(json.dumps(_hibp_email_lookup_to_dict(args.email, breaches), indent=2))
        return
    if not breaches:
        print(f"No HIBP breaches returned for {args.email}.")
        print(_hibp_email_lookup_boundary(False))
        return
    print(f"HIBP returned {len(breaches)} breach(es) for {args.email}:")
    print(_hibp_email_lookup_boundary(True))
    for breach in breaches:
        print(f"  - {breach.name}")


def _hibp_email_lookup_boundary(found: bool) -> str:
    if found:
        return (
    ...
    return (
        "Known: HIBP returned no breach records for this email. Unknown: that does not prove the email is "
        "absent from every breach; private breaches, dark-web dumps, private forums, and paid "
        "breach-intelligence sources outside HIBP may still exist."
    )


def _hibp_email_lookup_to_dict(email: str, breaches) -> dict[str, object]:
    return {
        "record_type": "hibp_email_breach_lookup",
        "email": email,
        "lookup_boundary": _hibp_email_lookup_boundary(bool(breaches)),
        "breaches": [breach.__dict__ for breach in breaches],
    }


def _pwned_password(args: argparse.Namespace) -> None:
    _require_old_or_reused_confirmation(args)
    password = get_secret(args.password_secret)
    if not password:
        raise SystemExit(_missing_secret_message("Password"))
    count = _pwned_password_count_or_exit(HibpBreachChecker(), password)
    AuditLogger().write("hibp_pwned_password_check", count=count)
    print(_password_exposure_scope_warning())
    print(_password_secret_cleanup_reminder())
    if count:
        print(
            f"Password appears {count} time(s) in HIBP Pwned Passwords. "
            "Do not reuse this password; review where it was used before rotating accounts."
        )
    else:
        print("Password was not found in HIBP Pwned Passwords.")
    print(_password_exposure_cli_summary(count))
...
def _exposure_plan(args: argparse.Namespace) -> None:
    ...
    if args.hibp_secret:
        ...
        breaches = _breaches_for_account_or_exit(checker, args.email)
    ...
    if args.password_secret:
        ...
        if not password:
            raise SystemExit(_missing_secret_message("Password"))
        pwned_count = _pwned_password_count_or_exit(checker, password)
...
def _rotate(args: argparse.Namespace) -> None:
    ...
    if args.copy_selected:
        if copy_text(selected.password, clear_after_seconds=60):
            ...
            raise SystemExit(
                "Clipboard copy is not available on this platform/session. Plaintext was not printed; rerun with "
                "--reveal-selected only if you accept terminal and screen exposure risk."
            )
    elif getattr(args, "reveal_selected", False) or args.reveal_all:
        print("Unsafe reveal requested; keep the terminal private and clear scrollback when finished.")
        print(f"Selected password: {selected.password}")
    else:
        raise SystemExit(
            "Selected password was not printed. Rerun with --copy-selected (recommended) or --reveal-selected "
            "only if you accept terminal and screen exposure risk."
        )
...
def _write_vaults(args: argparse.Namespace) -> None:
    password = get_secret(args.password_secret)
    if not password:
        raise SystemExit(_missing_secret_message("New password"))
...
def _finding_to_dict(finding) -> dict[str, object]:
    return {
        "record_type": "account_risk_signal",
        "interpretation": "Mailbox risk signal for review; not proof of account takeover.",
...
def _exposure_report_to_dict(report: ExposureReport) -> dict[str, object]:
    return {
        "record_type": "exposure_review_plan",
        "interpretation": (
            "This plan combines authorized mailbox evidence and HIBP results. It does not search the whole web, "
            "and private breach datasets, dark-web dumps, private forums, or paid intelligence sources "
        ...
        "password_pwned_count": report.password_pwned_count,
        "password_check_scope_warning": _password_exposure_scope_warning(),
        "password_secret_cleanup_reminder": (
            _password_secret_cleanup_reminder() if report.password_pwned_count is not None else None
        ),
        "rotation_count": report.rotation_count,
        ...
        "recommendations": [
            {
                ...
                "rotate": item.rotate,
                "action": _exposure_recommendation_action(item),
                "action_detail": _exposure_recommendation_action_detail(item),
...
def _exposure_recommendation_action(item) -> str:
    if item.rotate:
        return "rotate"
    if any("confirm this account used that password" in reason for reason in item.reasons):
        return "review_reuse"
    return "review"


def _exposure_recommendation_action_detail(item) -> str:
    if item.rotate:
        return "Rotate this account because a separate risk signal supports rotation."
    if _exposure_recommendation_action(item) == "review_reuse":
        return "Review whether this account used the checked password before rotating it."
    return "Review this account's evidence before deciding whether rotation is needed."


def _print_exposure_report(report: ExposureReport) -> None:
    print(f"Exposure review plan for {report.email_address}")
    ...
    if report.password_pwned_count is None:
        ...
        print(_password_secret_cleanup_reminder())
        print(
            f"Password exposure check: found {report.password_pwned_count} time(s); "
            "review where you reused it, then rotate only those accounts"
        )
        print(_password_exposure_cli_summary(report.password_pwned_count))
    else:
        print(_password_exposure_scope_warning())
        print(_password_secret_cleanup_reminder())
    ...
    if not report.recommendations:
        ...
        if report.password_pwned_count and report.password_pwned_count > 0:
            print("Fallback review steps:")
            print("  Use your password manager or memory to identify where this old password was reused.")
            print("  Rotate only those accounts, one at a time, starting with email and financial accounts.")
            print("  Do not paste the password into search engines, paste sites, or random breach-check pages.")
        return
    print("Rotation priorities:")
    for item in report.recommendations:
        action = _exposure_recommendation_action_label(item)
        domain = f" ({item.sender_domain})" if item.sender_domain else ""
        print(f"- [{item.priority}] {action}: {item.service_name}{domain}")
        print(f"  action: {_exposure_recommendation_action_detail(item)}")
        for reason in item.reasons:
            print(f"  reason: {reason}")


def _exposure_recommendation_action_label(item) -> str:
    action = _exposure_recommendation_action(item)
    if action == "rotate":
        return "ROTATE"
    if action == "review_reuse":
        return "review reuse"
    return "review"


def _password_exposure_cli_summary(count: int) -> str:
    if count > 0:
        return (
            "Known: this password appears in HIBP Pwned Passwords. Unknown: this does not identify every "
            "site where you reused it. Next: rotate only accounts where you used this password, one at a time."
        )
    return (
        "Known: this password was not found in HIBP Pwned Passwords. Unknown: private breach dumps and "
        "sources outside HIBP may still exist. Next: keep passwords unique and still review suspicious mailbox alerts."
    )


def _password_exposure_scope_warning() -> str:
    return "Only check an old or reused password. Do not check a new generated password."


def _password_secret_cleanup_reminder() -> str:
    return "When finished, delete the temporary password-check secret with: arg secret-delete <secret-name>"


def _require_old_or_reused_confirmation(args: argparse.Namespace) -> None:
    if not getattr(args, "confirm_old_or_reused", False):
        raise SystemExit(
            "Confirm this is an old or reused password with --confirm-old-or-reused. "
            "Do not check a new generated password."
        )


def _pwned_password_count_or_exit(checker: HibpBreachChecker, password: str) -> int:
    try:
        return checker.pwned_password_count(password)
    except Exception:
        raise SystemExit("Password exposure check could not finish. Try again later.") from None


def _breaches_for_account_or_exit(checker: HibpBreachChecker, email: str):
    try:
        return checker.breaches_for_account(email)
    except Exception:
        raise SystemExit("HIBP email breach lookup could not finish. Try again later.") from None
...
def _finding_from_dict(data: dict[str, object]):
    ...
    from .models import AccountRiskFinding

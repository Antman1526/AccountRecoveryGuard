...
def test_audit_logger_redacts_local_path_parent_directories(tmp_path):
    path = tmp_path / "audit.jsonl"

    AuditLogger(path).write(
        "nordpass_import_staged",
        path="/Users/Antman/Documents/Personal Recovery/nordpass-import.csv",
        csv_path="/Users/Antman/Exports/helper@example.com/nordpass-import.csv",
    )

    record = json.loads(path.read_text(encoding="utf-8"))
    serialized = json.dumps(record)
    assert "Antman" not in serialized
    assert "Personal Recovery" not in serialized
    assert "helper@example.com" not in serialized
    assert record["path"] == "[PATH_REDACTED]/nordpass-import.csv"
    assert record["csv_path"] == "[PATH_REDACTED]/nordpass-import.csv"


def test_audit_logger_redacts_windows_path_parent_directories(tmp_path):
    path = tmp_path / "audit.jsonl"

    AuditLogger(path).write(
        "nordpass_import_staged",
        path=r"C:\Users\Antman\Exports\helper@example.com\nordpass-import.csv",
    )

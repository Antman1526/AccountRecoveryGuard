...
from account_recovery_guard.gui_services import (
    ...
    SETUP_DETAIL_PROVIDER_PREP_FAILED,
...
def test_gmail_provider_factory_sanitizes_provider_constructor_failure(monkeypatch):
    secure_store = types.ModuleType("account_recovery_guard.secure_store")
    secure_store.set_secret = lambda name, value: None
    secure_store.get_secret = lambda name: "abcdefghijklmnop"
    monkeypatch.setitem(sys.modules, "account_recovery_guard.secure_store", secure_store)

    email_scanner = types.ModuleType("account_recovery_guard.email_scanner")
    ...
    class FakeScanner:
        def __init__(self, config):
            raise RuntimeError("imap failed password=super-secret-value")

    email_scanner.ImapMailboxConfig = FakeConfig
    email_scanner.ImapEmailScanner = FakeScanner
    monkeypatch.setitem(sys.modules, "account_recovery_guard.email_scanner", email_scanner)
    ...
    assert provider is None
    assert error is not None
    assert "Gmail setup could not be prepared" in error.user_message
    assert error.technical_details == SETUP_DETAIL_PROVIDER_PREP_FAILED
    assert "super-secret-value" not in error.user_message
    assert "super-secret-value" not in error.technical_details


def test_outlook_provider_factory_sanitizes_provider_constructor_failure(monkeypatch):
    oauth_mail = types.ModuleType("account_recovery_guard.oauth_mail")
    ...
    class FakeGraphProvider:
        def __init__(self, config):
            raise RuntimeError("graph failed access_token=super-secret-value")

    oauth_mail.GraphOAuthConfig = FakeGraphConfig
    oauth_mail.MicrosoftGraphMailProvider = FakeGraphProvider
    monkeypatch.setitem(sys.modules, "account_recovery_guard.oauth_mail", oauth_mail)

    provider, error = build_provider_or_error(
        MailProviderSettings(
            provider=MailProviderChoice.OUTLOOK,
    ...
    assert provider is None
    assert error is not None
    assert "Outlook setup could not be prepared" in error.user_message
    assert error.technical_details == SETUP_DETAIL_PROVIDER_PREP_FAILED
    assert "access_token" not in error.user_message
    assert "super-secret-value" not in error.technical_details
...
def test_password_exposure_service_reports_found_without_revealing_password():
    ...
    assert "Do not reuse it" in result.user_message
    assert "Do not use it again" not in result.user_message
    assert "review where you reused it" in result.user_message
    assert "rotate only those accounts" in result.user_message
    assert "rotate any account where you used it" not in result.user_message.lower()
...
def test_password_exposure_service_reuses_session_result_without_network_or_plaintext():
    ...
    assert "review where you reused it" in second.user_message.lower()
    assert "rotate only those accounts" in second.user_message
    assert "rotate accounts where you reused it" not in second.user_message.lower()

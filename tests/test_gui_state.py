...
from account_recovery_guard.models import AccountRiskFinding, DiscoveredAccount, PasswordCandidate
...
def test_first_run_checklist_points_to_account_review_when_findings_exist():
    ...
    assert "Review the highest-risk account evidence first" in checklist["Review one account"].detail
    assert "rotate only if" in checklist["Review one account"].detail
    assert "rotate one account at a time" not in checklist["Review one account"].detail
...
def test_password_exposure_count_updates_checklist_without_password_storage():
    ...
    assert "appears in HIBP Pwned Passwords" in exposed_check["Check password exposure"].detail
    assert "Review where you reused it before rotating only those accounts." in exposed_check["Check password exposure"].detail
    assert "appears in breach corpuses" not in exposed_check["Check password exposure"].detail
...
def test_password_exposure_rotation_guidance_is_actionable_without_overstating_detection():
    ...
    assert "review any suspicious alert" in clean.password_exposure_rotation_guidance
    assert "rotate only if" in clean.password_exposure_rotation_guidance
    assert "rotate any account" not in clean.password_exposure_rotation_guidance
    assert "appears in HIBP Pwned Passwords" in exposed.password_exposure_rotation_guidance
    assert "Review where you reused it" in exposed.password_exposure_rotation_guidance
    ...
    assert "breach corpuses" not in exposed.password_exposure_rotation_guidance
...
def test_review_subject_label_is_bounded_and_nonempty():
    state = GuiAppState.new().with_review_subject("  spouse   mailbox  ")

    assert state.review_subject_label == "spouse mailbox"
    assert state.review_subject_prefix == "spouse mailbox: "
...
def test_scan_owner_records_person_and_mailbox_for_guided_rotation():
    ...
    assert state.review_subject_label == "second person"
...
def test_blank_review_subject_label_defaults_to_me():
    state = GuiAppState.new().with_review_subject("   ")

    assert state.review_subject_label == "Me"
...
def test_second_person_consent_is_required_only_for_non_me_labels():
    ...
    assert GuiAppState.new().with_review_subject("spouse mailbox").requires_second_person_consent is True


def test_long_review_subject_label_is_bounded():
    state = GuiAppState.new().with_review_subject("x" * 100)

    assert len(state.review_subject_label) == 40
...
def test_scan_summary_counts_repeated_alerts_as_one_account_with_merged_reasons():
    suspicious_login = AccountRiskFinding(
        service_name="dropbox",
        sender_domain="dropbox.com",
        sender="security@dropbox.com",
        subject="Suspicious login",
    ...
    password_reset = AccountRiskFinding(
        service_name="Dropbox",
        sender_domain="dropbox.com",
        sender="security@dropbox.com",
    ...
    summary = ScanSummary.from_findings([password_reset, suspicious_login], discovered_count=1)
    reviews = summary.account_reviews("me@example.com")

    assert len(summary.findings) == 2
    assert summary.accounts_needing_attention == 1
    assert summary.attention_text == "1 account needs attention."
    assert len(reviews) == 1
    assert reviews[0].service_name == "Dropbox"
    assert reviews[0].risk_label == "Needs attention"
    assert reviews[0].reset_link == "https://dropbox.com/reset"
    assert reviews[0].reasons == ("suspicious activity", "password reset")
...
def test_scan_summary_linked_accounts_explanation_prevents_rotation_panic():
    summary = ScanSummary.from_findings([], discovered_count=1)
    explanation = summary.linked_accounts_explanation.lower()

    assert "authorized mailbox evidence only" in explanation
    assert "not exposure findings" in explanation
    assert "do not need to change their passwords" in explanation
    assert "unless" in explanation
    assert "all sites" not in explanation
    assert "hunter2" not in explanation


def test_scan_summary_next_review_action_is_plain_language():
    finding = AccountRiskFinding(
        service_name="dropbox",
        sender_domain="dropbox.com",
        sender="security@dropbox.com",
    ...
    assert "Start with Dropbox" in summary.next_review_action
    assert "rotate only that account first" in summary.next_review_action
    assert "all accounts" not in summary.next_review_action.lower()
    assert "hunter2" not in summary.next_review_action.lower()


def test_empty_scan_summary_next_review_action_avoids_rotation_pressure():
    summary = ScanSummary.from_findings([], discovered_count=0)

    assert "No urgent alerts" in summary.next_review_action
    assert "monitoring" in summary.next_review_action
...
def test_scan_summary_interpretation_does_not_overstate_no_alerts():
    ...
    assert "does not prove every password is risk-free" in summary.interpretation
    assert "does not prove every password is safe" not in summary.interpretation

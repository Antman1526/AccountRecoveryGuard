...
class ResetBrowserWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.failed.emit("The protected recovery browser could not open. Use the official website or app instead.")
            return
        self.finished.emit("Recovery browser closed. Confirm the new password works before preparing vault sync.")

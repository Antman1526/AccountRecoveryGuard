...
def main() -> int:
    ...
    class MainWindow(QMainWindow):
        ...
        def _advanced_rotation_page_unreachable_from_first_run(self) -> QScrollArea:
            ...
            form.addWidget(official_page_confirmed, 6, 1)
            ...
            button_row.addStretch(1)
            form.addLayout(button_row, 7, 1)
            form.addWidget(QLabel("Password choices"), 8, 0)
            form.addWidget(choices, 8, 1)
            form.addWidget(QLabel("Status"), 9, 0)
            form.addWidget(result, 9, 1)
            form.addWidget(QLabel("CLI equivalent"), 10, 0)
            form.addWidget(preview, 10, 1)
            form.addWidget(self._copy_button("Copy rotate command", preview.toPlainText), 11, 1, alignment=Qt.AlignmentFlag.AlignRight)
            layout.addWidget(panel)
            update_preview()
            return page
